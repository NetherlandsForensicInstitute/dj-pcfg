...
class PcfgGuesserMultithreadedTest {
    ...
    class TestWithOriginalRule {
        ...
        void generateLargeAmountOfPasswords() throws IOException, InterruptedException {
            ...
            final List<String> actual = generateParallel(rulePath, 0, 10000);
        ...
        void skipWithLimit1(final String rulePath, final int skip) throws IOException, InterruptedException {
            final List<String> expectedNoSkip = generateWithPython(rulePath, skip + 1);

            final List<String> expected = expectedNoSkip.subList(skip, expectedNoSkip.size());
            final List<String> actual = generateParallel(rulePath, skip, 1);
        ...
        void skipWithLimit13(final String rulePath, final int skip) throws IOException, InterruptedException {
            final List<String> expectedNoSkip = generateWithPython(rulePath, skip + 13);

            final List<String> expected = expectedNoSkip.subList(skip, expectedNoSkip.size());
            final List<String> actual = generateParallel(rulePath, skip, 13);
        ...
        void skipWithLimit137(final String rulePath, final int skip) throws IOException, InterruptedException {
            final List<String> expectedNoSkip = generateWithPython(rulePath, skip + 137);

            final List<String> expected = expectedNoSkip.subList(skip, expectedNoSkip.size());
            final List<String> actual = generateParallel(rulePath, skip, 137);
    ...
    class TestWithSerializedRule {
        ...
        void generateLargeAmountOfPasswords() throws IOException, InterruptedException {
            final String rulePath = "rules/Default";
            final List<String> expected = generateWithPython(rulePath, 10000);
            final List<String> actual = generateParallel(rulePath + ".pbm", 0, 10000);
        ...
        void skipWithLimit1(final String rulePath, final int skip) throws IOException {
            assertThat(generateParallel(rulePath, skip, 1))
        ...
        void skipWithLimit13(final String rulePath, final int skip) throws IOException {
            assertThat(generateParallel(rulePath, skip, 13).stream().sorted().toList())
                    .isEqualTo(generate("rules/Default", skip, 13).stream().sorted().toList());
        }
        ...
        void skipWithLimit137(final String rulePath, final int skip) throws IOException {
            assertThat(generateParallel(rulePath, skip, 137).stream().sorted().toList())
                    .isEqualTo(generate("rules/Default", skip, 137).stream().sorted().toList());
        }
    }
    ...
    class CacheTest {
        ...
        void skipPairsEmptySizeCache(final String rulePath, final int skipBefore, final int skipAfter) throws IOException {
            ...
            // sanity check
            assertThat(expected.size()).isEqualTo(FIXED_LIMIT);

            List<String> actual;
            actual = generateParallel(guesser.cache(cache), skipBefore, FIXED_LIMIT);
            actual = generateParallel(guesser.cache(cache), skipAfter, FIXED_LIMIT);
    ...
    private static List<String> generateParallel(final String rulePath, final long skip, final long limit) throws IOException {
        return generateParallel(PcfgGuesser.forRule(TEST_RESOURCES_PATH.resolve(rulePath)), skip, limit);
    }

    private static List<String> generateParallel(final PcfgGuesser guesser, final long skip, final long limit) throws IOException {
        final ByteArrayOutputStream output = new ByteArrayOutputStream();
        guesser.threadCount(4).generateGuesses(skip, limit, new PrintStream(output));
